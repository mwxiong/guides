...
Emitting has a connotation of outward flow, while triggering has a connotation of inward flow. (DF: Inward meaning what?  Parent to child?)
...
(DF: Wouldn't this be pulled downstream?)
...
Hence, you should subscribe to Surfaces and the Engine (which can both listen to the DOM),
whereas you should pipe from (or subscribe to) custom events broadcast from widgets.
...
```
(DF Comment: Do intermediary handlers need to subscribe to event foo for the last handler to effectively subscribe to foo? )
...
and their interface is similar.
(DF COmment: Let's see if we can avoid the use of 'Widget' in the tutorial). However, event handling in widgets is complicated
